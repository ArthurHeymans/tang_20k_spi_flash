    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (spi_cmd_write_buf[1] && !spi_write_ack && spi_csel_buf[1]) begin
                ...
                // State 0 = page program (prepare data from buffer)
                // State 2 = erase (write 0xFF, skip data preparation)
            ...
            // Page program (type=0): 0 -> 1 -> 2 -> 3 -> 4 -> 0 (loop) or 3 -> 5 (done)
            // Erase (type=1): starts at 2 -> 3 -> 4 -> 2 (loop) or 3 -> 5 (done)
            if (spi_writing && !sdram_busy) begin
                if (i_spi_write_state == 0) begin
                    ...
                    // Wait state to let write_buffer register before SDRAM access
                    i_spi_write_state <= 1;
                end
                    ...
                    // Erase: loop back to activate (state 2)
                    // Page program: loop back to prepare data (state 0)
